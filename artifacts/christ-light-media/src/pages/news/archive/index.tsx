...
type Filters = {
  ...
  dateRange: string;
  sort: string;
  year: string; // For archive, we'll have year filter
};

const DEFAULT_LIMIT = 12; // More items per load for archive list view
...
export default function NewsArchivePage() {
  ...
  useEffect(() => {
    async function fetchFilters() {
      try {
        const response = await fetch(`/api/news/archive?limit=1`);
  ...
  const fetchArticles = useCallback(async (reset = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/news/archive?${queryParams}`);
      const data = await response.json();
  ...
  }, [filters.category, debouncedSearch, filters.dateRange, filters.sort, filters.year, offset]);

  // Reset pagination when filters change
  useEffect(() => {
    fetchArticles(true);
  }, [filters.category, filters.search, filters.dateRange, filters.sort, filters.year, fetchArticles]);
    ...
    return (
      <div className="min-h-screen bg-[#0A0A0A]">
        <PageHeader
          label="ARCHIVE"
          title="News Archive"
          description="Browse all Christian news and ministry updates"
        />
      ...
      <section className="pb-20">
        <div className="container mx-auto px-6">
          ...
          <div className="mb-8 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="relative">
                ...
                <div className="absolute left-0 mt-2 w-56 rounded-md bg-card border border-white/10 py-2 z-20 hidden">
                  <button
                    ...
                    className={`flex w-full items-center px-4 py-2 text-sm ${filters.year === 'all' ? 'bg-gold text-black' : 'text-white hover:bg-white/10'}`}
                  >
                    All Years
                  </button>
                  {years.map((year) => (
                    <button
                      key={year}
                      onClick={() => setFilters(prev => ({ ...prev, year: year }))}
                      className={`flex w-full items-center px-4 py-2 text-sm ${filters.year === year ? 'bg-gold text-black' : 'text-white hover:bg-white/10'}`}
                    >
                      {year}
                    </button>
                  ))}
            ...
            {/* Sort Options */}
            <div className="w-full lg:w-auto min-w-[200px] text-right">
              <div className="relative">
          ...
          {!loading && articles.length === 0 && offset === 0 && (
            <div className="text-center py-20">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <button 
                  onClick={() => setFilters(prev => ({ ...prev, search: '', category: 'all', year: 'all', dateRange: 'all' }))}
